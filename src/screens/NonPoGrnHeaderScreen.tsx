...
import { getStockReportStores, getSuppliers, postNonPoReceiptHeader, type StockReportStoreItem, type SupplierListItem } from '../api';
...
export function NonPoGrnHeaderScreen() {
  ...
  const onSave = async () => {
    ...
    setError(null);
    try {
      const res = await postNonPoReceiptHeader(token, {
      ...
      navigation.replace('NonPoReceiptWorkspace', {
        receiptId: res.data.id,
        initialTab: 'lines',
      });
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Could not save receipt.';
      setError(message);
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
          <Text style={{ ...outfit('regular', 13), color: colors.textSecondary, marginBottom: 14 }}>
            Save the header first, then add lines on the receipt workspace (same as web ERP).
          </Text>
          ...
          <FormSection title="Receipt details">
            <ReadonlyField label="Received date" value={formatDisplayDate(receivedDate)} hint="Recorded as today's date." />
