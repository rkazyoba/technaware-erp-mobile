...
import { NonPoReceiptOverviewPanel, type NonPoHeaderForm } from '../components/nonpo/NonPoReceiptOverviewPanel';
...
import {
  deleteNonPoReceiptLine,
  getCatalogUnitPrice,
  getLogisticsDocDetail,
  getNonPoReceiptLineCategories,
  getNonPoReceiptLineStoreItems,
  getStockReportStores,
  getSuppliers,
  postNonPoReceiptLine,
  putNonPoReceiptHeader,
  putNonPoReceiptLine,
  type LogisticsDocDetail,
  type NonPoLineStoreItem,
...
const BASE_PATH = 'inventory/non-po-receipts';
const TAB_OVERVIEW = 'overview';
const TAB_LINES = 'lines';
const TAB_DOCUMENT = 'document';

function headerFormFromDetail(d: LogisticsDocDetail): NonPoHeaderForm {
  return {
...
export function NonPoReceiptWorkspaceScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<ModulesStackParamList>>();
  const route = useRoute<RouteProp<ModulesStackParamList, 'NonPoReceiptWorkspace'>>();
  ...
  const moduleGate = useMemo(() => portalModuleAccessGate(portal, 'Non-PO receipts'), [portal]);
  ...
  const [headerForm, setHeaderForm] = useState<NonPoHeaderForm | null>(null);
  ...
  const [lineStoreItemsById, setLineStoreItemsById] = useState<Record<string, NonPoLineStoreItem>>({});
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      setPortalSelectedModule('Non-PO receipts');
  ...
  const saveHeader = async () => {
    ...
    try {
      await putNonPoReceiptHeader(token, receiptId, {
  ...
  const storeItemToStockRow = (item: NonPoLineStoreItem, categoryLabel: string): StockReportLine => ({
    id: item.id,
    code: item.code,
  ...
  const openStockPicker = async () => {
    ...
    try {
      const res = await getNonPoReceiptLineCategories(token);
  ...
  const onSelectLineCategory = async (opt: SearchableSelectOption) => {
    ...
    setLinePickError(null);
    try {
      const res = await getNonPoReceiptLineStoreItems(token, storeId, opt.id);
      const byId: Record<string, NonPoLineStoreItem> = {};
  ...
  const onSelectLineItem = async (opt: SearchableSelectOption) => {
    ...
    const asOf = headerForm?.receivedDate || detail?.document_date?.slice(0, 10);
    try {
  ...
  const submitLine = async () => {
    ...
    try {
      if (editingLineId) {
        await putNonPoReceiptLine(token, receiptId, editingLineId, {
          received_qty: qty,
        ...
        await postNonPoReceiptLine(token, receiptId, {
          category_id: Number(selectedStock.category_id),
          received_item: Number(selectedStock.id),
  ...
  const deleteLine = async (lineId: string) => {
    ...
    try {
      await deleteNonPoReceiptLine(token, receiptId, lineId);
      await loadDetail(true);
    } catch (e) {
  ...
  if (moduleGate === 'denied' || !isPortalModuleRouteAccessible(portal, 'Non-PO receipts')) {
    return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.pageBg }}>
      <TopBar
        title={detail?.ref ?? 'Non-PO receipt'}
