...
import { FormSection, ReadonlyField } from '../SearchableSelectField';
...
type Props = {
  ...
  editable: boolean;
  form: PoHeaderForm;
  onChangeForm: (patch: Partial<PoHeaderForm>) => void;
  onSaveHeader?: () => void;
...
export function PoReceiptOverviewPanel({
  detail,
  lineCount,
  editable,
  form,
  ...
  const orderDisplay = form.orderLabel || detail.order_no || detail.context?.match(/PO #([^·]+)/)?.[1]?.trim() || '—';
  ...
  return (
    <View>
      ...
      >
        ...
        {detail.expiration_summary ? (
          <Text style={{ ...outfit('regular', 12), color: colors.textMuted, marginTop: 8 }}>
            Lot / expiry: {detail.expiration_summary.allocated_qty.toFixed(2)} of{' '}
            {detail.expiration_summary.received_qty.toFixed(2)} allocated
            {detail.expiration_summary.complete ? ' (complete)' : ''}
          </Text>
        ) : null}
        <View
          style={{
      ...
      {editable ? (
        <>
          <FormSection title="Identification">
            <ReadonlyField label="Receipt no. (GRN)" value={detail.ref} />
          ...
          <FormSection title="Receipt details">
            <ReadonlyField label="Purchase order" value={orderDisplay} hint="Change the PO from web ERP if needed." />
            <DatePickerField
              label="Received date"
              ...
              marginTop={0}
            />
            <ReadonlyField label="Supplier" value={supplierDisplay} />
            <ReadonlyField label="Site" value={siteDisplay} />
            <ReadonlyField label="Store" value={storeDisplay} />
        ...
        >
          ...
          <MetaRow label="Purchase order" value={orderDisplay} />
